...
收集完毕后，生成以下文件（初始值如下）：
- `digital_human/persona.md`
- `digital_human/mode.md`：
  - 舔狗模式：`current_position = -70`，`target_position = -70`，`range_regression = 40`，`range_progression = 20`
  - 海王模式：`current_position = +70`，`target_position = +70`，`range_regression = 20`，`range_progression = 40`
- `digital_human/growth_log.md`（空文件，待追加）
- `digital_human/self_worth_tracker.md`
...
用户可随时说：
- `"切换目标模式：海王"` → `target_position = +60`，数字人开始渐变
- `"切换目标模式：舔狗"` → `target_position = -60`，数字人开始渐变
- `"追加记录"` → 更新对手方画像
- `"这不对，我不会这样说"` → 触发 `prompts/correction_handler.md`（永久修正）
- `"换一种方式"` → 临时调整，不写入修正层
...
每轮对话结束后自动执行 `prompts/debrief.md`（含触发事件识别、位移计算、范围约束、文件更新、复盘摘要展示）。
